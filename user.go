...
func LogInUser(w http.ResponseWriter, r *http.Request, p httprouter.Params) string {
	...
	u := User{}
	res, err := rdb.Table("users").GetAll(p.ByName("username")).Limit(1).Run(rdbSession)
	if err != nil {
		log.Println(err)
	}
	err = res.One(&u)
	fmt.Println("RDB Res: ", res)
	bytePass := []byte(p.ByName("password"))
	if (u.Username == "") || (bcrypt.CompareHashAndPassword(u.Password, bytePass) != nil) {
